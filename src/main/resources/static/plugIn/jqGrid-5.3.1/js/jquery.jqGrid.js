...
$.extend($.jgrid,{
	...
	parse : function(jsonString) {
		var js = jsonString.replace(/\t/g, ''); // 清除搜索字符串中的tab符，防止搜索报错
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		// 2021-03-22从localstorage中拿显示的表头数据---syp
		if(pin.storeHeaderKey) { // 含将显示的表头数据存入localstorage的配置
			var storeHeader = localStorage.getItem(pin.storeHeaderKey)
			if (storeHeader) {
				for (var curHead of pin.colModel) {
					if (JSON.parse(storeHeader).indexOf(curHead.name) > -1) { // 要显示的列
						curHead.hidden = false;
					} else { // 要隐藏的列
						curHead.hidden = true;
					}
				}
		...
		addLocalData = function( retAll ) {
			...
			retresult = {};
			window.filterResult = queryResults;
		...
		buildColItems = function (top, left, parent) {
			...
			str1 = '<ul id="col_menu" class="ui-search-menu  ui-col-menu modal-content" role="menu" tabindex="0" style="left:'+left+'px; top:'+top+'px; padding-top: 0;">';
			// 设置多级表头的名字区分
			var lb = [];
			if (ts.p.groupHeader) { // 若是多级表头
				switch (ts.p.groupHeader.length) {
					case 1 : { // 二级表头
						var oneCol = ts.p.groupHeader[0].groupHeaders;
						var twoCol = cm;
						var currentColEnd = 0;
						for (var i = 0; i < oneCol.length; i++) {
							var startKey = oneCol[i]["startColumnName"];
							var flag = false;
							for (var j = currentColEnd; j < twoCol.length; j++) {
								var key = twoCol[j]["name"];
								if (startKey == key) {
									currentColEnd = j + parseInt(oneCol[i]["numberOfColumns"]);
									flag = true;
								}
								if (flag) {
									if (j == currentColEnd) {
									...
									lb.push(oneCol[i]["titleText"] + "-" + twoCol[j]["label"]);
								} else {
									lb.push(twoCol[j]["label"]);
								}
							}
						}
						// 若一级表头循环完，还有二级表头未循环到
						if (currentColEnd <= twoCol.length - 1) {
							for (var i = currentColEnd; i < twoCol.length; i++) {
								lb.push(twoCol[i]["label"]);
							}
						}
					}
				}
			...
			// 添加全选checkbox
			str1 += '<li style="position: sticky; top: 0; padding: 10px 0 0 0; background-color: #fff; z-index: 10;"><table draggable="false" style="width: 100%;"><tbody><tr><td class="menu_icon" style="padding-right: 0;">';
			// 全选checkbox状态，默认选中
			var mutiCheck = "checked";
			// 确定全选按钮初始状态是否选中
			for (var colObj of cm) {
				if (colObj["hidden"] && !colObj["hidedlg"]) {
					mutiCheck = "";
					break;
				}
			}
			str1 += '<input type="checkbox" ' + mutiCheck + ' name="muti_select_thead" id=mutiSelect_' + ts.p.id + ' onclick="mutiSelectThead(this);" style="margin-right: 5px;">';
			str1 += '<label for=mutiSelect_' + ts.p.id + ' class="cp c111" style="margin-bottom: 0;">全选</label></td>';
			str1 += '<td class="menu_icon fr" style="padding-right: 0;"><button type="button" class="btn btn-default btn-xs" onclick="mutiSelectTheadSure(this, \'' + ts.p.id + '\');">确定</button></td></tr></tbody></table></li>';
			str1 += '<li style="padding: 0"><ul>'
			for(i=0;i<len;i++) {
				//if(!cm[i].hidedlg) { // column chooser
//				var hid = !cm[i].hidden ? "checked" : "", nm = cm[i].name, lb = ts.p.colNames[i];
				var hid = !cm[i].hidden ? "checked" : "", nm = cm[i].name;
				disp = (nm === 'cb' || nm==='subgrid' || nm==='rn' || cm[i].hidedlg) ? "style='display:none'" :"";
				str1 += '<li '+disp+' class="ui-menu-item" role="presentation" draggable="true"><a class="g-menu-item" tabindex="0" role="menuitem" ><table class="ui-common-table" ><tr><td class="menu_icon" title="'+texts.reorder+'"><span class="'+iconbase+' '+colmenustyle.icon_move+' notclick"></span></td><td class="menu_icon"><input class="'+colmenustyle.input_checkbox+'" type="checkbox" name="'+nm+'" '+hid+'></td><td class="menu_text">'+lb[i]+'</td></tr></table></a></li>';
				cols.push(i);
			}
			str1 += "</ul></li></ul>";
			$(parent).append(str1);
			...
			$("#col_menu > li > ul > li > a").on("click", function(e) {
				...
				if ($checkbox.prop('checked')) {
					if ($checkbox.closest('ul').find('input[type="checkbox"]:checked').length == $checkbox.closest('ul').find('input[type="checkbox"]').length) { // 若选中的checkbox个数等于总数，则全选checkbox选中
						$checkbox.closest('ul').closest('li').siblings('li').find('input[type="checkbox"]').prop('checked', true);
					}
				} else {
					$checkbox.closest('ul').closest('li').siblings('li').find('input[type="checkbox"]').prop('checked', false);
				}
...
//					$(ts).jqGrid('showCol', col_name );
		...
		if(ts.p.colMenu || ts.p.menubar) {
			$("body").on('click', function(e){
				...
				if(!$(e.target).closest(".custom-columns > li > a").length && !$(e.target).closest("#col_menu").length) {
					try {
					$("#col_menu").remove();
					} catch (e) {}
				}
		...
		// 默认生成tool_bar
		if($('#toolBar_' + ts.p.id).length == 0) {
			$('#gbox_' + ts.p.id).before('<div id="toolBar_' + ts.p.id + '" class="toolBar-jqgrid form-inline mb5"></div>');
		}
		...
		if (ts.p.colMenu) {
			if($('#toolBar_' + ts.p.id).length == 0) {
				$('#gbox_' + ts.p.id).before('<div id="toolBar_' + ts.p.id + '" class="form-inline mb5"></div>');
			}
			var customColumn = '<li class="ui-menu-item" role="presentation"><a class="g-menu-item btn btn-default btn-xs" tabindex="0" role="menuitem" data-value="columns"><table class="ui-common-table"><tr><td class="menu_icon"><span class="glyphicon glyphicon-list-alt"></span></td><td class="menu_text">自定义表头</td></tr></table></a></li>'
			if ($('#toolBar_' + ts.p.id).find('.custom-columns').length == 0) {
				$('#toolBar_' + ts.p.id).prepend('<ul class="custom-columns dilb" style="position: relative; margin-right: 5px;">' + customColumn + '</ul>');
			}
			$('#toolBar_' + ts.p.id + " > .custom-columns > li > a").click(function(){
				$('#col_menu').remove();
				if ($('#col_menu').length == 0) {
					var left1, top1;
					if($(this).attr("data-value") === 'columns') {
						buildColItems(37, 0, $(this).parent());
					}
				}
			});
		}
		if (typeof(ts.p.globalSearch) != 'undefined' || typeof(ts.p.serveSearch) != 'undefined') { // 含前台搜索和后台搜索
			if($('#toolBar_' + ts.p.id).length == 0) {
				$('#gbox_' + ts.p.id).before('<div id="toolBar_' + ts.p.id + '" class="form-inline mb5"></div>');
			}
			var search = '<div class="input-group fr"><input id="' + ts.p.id + '_search" type="text" class="form-control" placeholder="数据匹配" />'
						+ '<div class="input-group-addon"><i class="glyphicon glyphicon-search"></i></div></div>';
			if ($('#' + ts.p.id + '_search').length == 0) {
				$('#toolBar_' + ts.p.id).append(search);
			}
			$('#toolBar_' + ts.p.id).height('32');
			// 前台搜索
			if (ts.p.globalSearch) {
				// 表格渲染过的数据
				var tableViewData = []; // 渲染数据
				var tableRealData = []; // 真实数据
				var $table = $('#' + ts.p.id);
//				var reg = /<[^>]+>/gim; // 匹配html标签的正则表达式
				var reg = /<([a-z]+)([^<>]+)/; // 匹配html标签的正则表达式
				var reg = /<\s*\/?\s*[a-zA-z_]([^>]*?["][^"]*["])*[^>"]*>/g; // 匹配html标签的正则表达式
				var timer = setInterval(function() { // 通过定时器第一时间得到渲染后的表格数据
					var rowNum = $table.jqGrid('getGridParam', 'rowNum');
					var total = $table.jqGrid('getGridParam', 'records');
					if (typeof(ts) != 'undefined' && typeof(ts.p) != 'undefined' && typeof(ts.p.globalSearch) != 'undefined') {
						if (total == ts.p.globalSearch.length) { // 现在有渲染后的所有数据
							window.clearInterval(timer);
							var cur,idr,v;
							var rowReader = reader('local');
							var gi = ts.p.multiselect ? 1 : 0;
							for(var i = 0; i < ts.p.globalSearch.length; i++) {
								var rd = {};
								cur = ts.p.globalSearch[i];
								idr = $.jgrid.getAccessor(cur,"id");
								if (idr === undefined) {
									idr = i + 1;
								}
								idr = ts.p.idPrefix + idr;
								for (var j=0;j<rowReader.length;j++) {
									v = $.jgrid.getAccessor(cur,rowReader[j]); // colModel的key对应的原始value
									// rd[ts.p.colModel[j+gi].name] = v;
//									rowData.push( addCell(idr,v,j+gi+ni+si,i+rcnt,cur, rd) );
									if (!ts.p.colModel[j+gi].hidden) { // 只存放初始未被隐藏的数据
										if(ts.p.colModel[j+gi].formatter !== undefined) {
											rd[ts.p.colModel[j+gi].name] = formatter(idr, v, j+gi, cur, "add"); // 渲染后的数据
										} else {
											if (typeof v == "undefined") {
												rd[ts.p.colModel[j+gi].name] = "";
											} else {
												rd[ts.p.colModel[j+gi].name] = v;
											}
										}
								...
								tableViewData.push(rd);
							}
							tableRealData = ts.p.globalSearch;
						}
					} else { // 防止操作过快，jqgrid反应不及的时候报错
						window.clearInterval(timer);
					}
				}, 100);
				// 30秒之后自动清除定时器
				...
				// 绑定搜索的input事件
				$('#' + ts.p.id + '_search').on('input', function() {
					var str = $(this).val();
					var newRowsData = [];
//	                if (str == "") { // 搜索框为空，则赋全部数据
//	                  newRowsData = tableRealData;
//	                } else {
//	                   var formObjArr = [];
					// 若含有input框
					if (ts.p.searchInput) {
						for (var i = 0; i < tableViewData.length; i++) {
						    var matchFlag = false; // 是否匹配到本行
						    var matchIndex = 0;
						    $.each(tableViewData[i], function(key, val) {
						        var textVal = val.toString();
						        if (reg.test(textVal)) {
						            // 若是html元素，则过滤掉标签部分，只留文本内容
						            // 判断是否含有input对象
						            var currentInputId = "";
						            var tagName = $(textVal)[0].tagName;
						            if (tagName == "INPUT") { // 若本身就是input元素
						                currentInputId = $(textVal).attr("id");
						                if (typeof($('#' + currentInputId)[0]) != 'undefined') {
						                    textVal = $('#' + currentInputId).val();
						                    tableRealData[i][key] = textVal; // 可用id查到的，证明是目前表格中可见的，需更新数据
						                } else { // 用id查不到的，证明是目前表格中不可见的，不需更新数据
						                    textVal = $(textVal).val();
						                }
						            } else if ($(textVal).find('input').length > 0) { // 若含input元素
						                currentInputId = $(textVal).find('input:eq(0)').attr('id');
						                if (typeof($('#' + currentInputId)[0]) != 'undefined') {
						                    textVal = $('#' + currentInputId).val();
						                    tableRealData[i][key] = textVal; // 可用id查到的，证明是目前表格中可见的，需更新数据
						                } else { // 用id查不到的，证明是目前表格中不可见的，不需更新数据
						                    textVal = $(textVal).find('input:eq(0)').val();
						                }
						            } else { // 若无input元素
						                // 匹配dom元素内容，若是显示的字段且与搜索值相匹配，则记录
						                textVal = textVal.replace(reg, "");
						            }
						        }
						        // 若匹配到则将flag设为true
						        if (str == "" || textVal.toLowerCase().indexOf(str.trim().toLowerCase()) > -1) {
						            matchFlag = true;
						        }
						        matchIndex += 1;
						        // 若最后一个属性遍历完成且flag为true，则将本行数据添加至newRowsData
						        if (matchIndex == Object.keys(tableViewData[i]).length && matchFlag) {
						            newRowsData.push(tableRealData[i]);
						        }
						    });
						}
	               } else { // 若不含input框
	               ...
	               setTimeout(function(){
	                 $('#' + ts.p.id).jqGrid('clearGridData');
	                 $('#' + ts.p.id).jqGrid('setGridParam',{ // 重新加载数据
	                       datatype:'local',
	                       data : newRowsData,
	                       page : 1
	                 }).trigger("reloadGrid");
			...
			// 后台搜索
			if (ts.p.serveSearch) {
				var postParam = ts.p.postData;
				// 绑定搜索的input事件
				$('#' + ts.p.id + '_search').on('input', function() {
					postParam["searchValue"] = $(this).val();
					$('#' + ts.p.id).jqGrid("setGridParam",{postData:postParam, page:1}).trigger("reloadGrid");
				});
			}
		}
		thead += "</tr></thead>";
		...
		$("th",thr).each(function ( j ) {
			...
			var ci;
			var $currentTh = $(this);
			var $currentTable = $(this).parents('table:eq(0)');
			if ($currentTable.parent('div.frozen-div').length > 0) { // 冻结列
				var thIndex = 0;
				$currentTable.find('th[id]').each(function(index, th) {
		...
		// 添加鼠标进入事件
		$(ts).on('mouseenter', 'tr', function(e){
			if(!$(this).hasClass('jqgrow')){return;}
			if (ts.p.moseenterRow) {
				ts.p.moseenterRow.call(ts, $(this).attr("id"), e);
			}
		});
		// 添加鼠标移出事件
		$(ts).on('mouseleave', 'tr', function(e){
			if(!$(this).hasClass('jqgrow')){return;}
			if (ts.p.mouseleaveRow) {
				ts.p.mouseleaveRow.call(ts, $(this).attr("id"), e);
			}
		});
		// 添加鼠标移动事件
		$(ts).on('mousemove', 'tr', function(e){
			if(!$(this).hasClass('jqgrow')){return;}
			if (ts.p.mousemoveRow) {
				ts.p.mousemoveRow.call(ts, $(this).attr("id"), e);
			}
		});
...
//				$t.p.cellEdit === true ||
	...
	destroyFrozenColumns :  function() {
		return this.each(function() {
			if ( !this.grid ) {return;}
			var $t = this;
			var hover = $($t).jqGrid('getStyleUI',$t.p.styleUI+".common",'hover', true);
...
$.jgrid.extend({
	editCell : function (iRow,iCol, ed, event){
		return this.each(function (){
			...
			// 此代码会导致在模态框中，点击可编辑的jqgrid后，模态框滚动；由于单元格编辑的保存方式已经重写，故此代码可注释
//			if (!$t.p.knv) {$($t).jqGrid("GridNav");}
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell") && (!$.isFunction($t.p.isCellEditable) || $t.p.isCellEditable.call($t,nm,iRow,iCol))) {
				...
				// 给input框添加placeholder
				$(elc).val('').attr('placeholder', tmp);
				cc.html("").append(elc).attr("tabindex","0");
				// 添加清空input框
				cc.append("<span class='clear-editable-cell'>x</span>");
				...
				var isDelete = false;
				$("span.clear-editable-cell",cc).on("mousedown",function(e) {
					$(this).prev('input').val('');
					isDelete = true;
					e.stopPropagation();
				});
				
				// 监听单元格编辑框change事件以自动保存
				$('input', cc).on("blur",function(e) {
					if (!isDelete) { // 若是删除引发的blur则不执行任何操作
						if ($(this).val() == '') { // 没有手动输入
							$($t).jqGrid("restoreCell",iRow,iCol);
						} else {
		...
		return this.each(function(){
			...
			if(fr !== null) {
				...
				if (v2 !== $t.p.savedRow[fr].v){
					...
					if(cv[0] === true) {
						if (typeof($t.p.savedRow[fr]) != 'undefined') {
					...
					case "select":
						if(p.autosearch === true) {
							soptions.dataEvents.push({
								type : "change",
								fn : function() {
									/*** 将选中的下拉列表的option上添加selected属性，使得锁定和解锁表头对应的选项一致 ***/
									var curValue = $(this).val(); // 当前选中的值
									var curSelectId = $(this).attr('id'); // 当前select的id
									var $selects = $('#gbox_' + $t.p.id).find('select[id="' + curSelectId + '"]'); // 选中当前表格容器下所有拥有此id的select
...
$.jgrid.extend({
	...
	setGroupHeaders : function ( o ) {
		...
		return this.each(function(){
			...
			$('#toolBar_' + ts.p.id).addClass('mulThead');
		});
	},
...
$.fn.html5sortable = function(options) {
	...
	return this.each(function() {
		...
		var isHandle, index; 
		items = $(this).children(options.items).not('table');
...
/**
 * 自定义表头全选方法，checkbox选中/不选中
 */
function mutiSelectThead(el) {
	var $checkboxs = $(el).closest('li').siblings('li').find('input[type="checkbox"]');
	if ($checkboxs.length > 0) {
		if ($(el).prop('checked')) { // 全选
 ...
 * 自定义表头全选方法，列隐藏/显示
 */
function mutiSelectTheadSure(el, tableId) {
	var $checkboxs = $(el).closest('li').siblings('li').find('input[type="checkbox"]');
	var tableHeader = $('#' + tableId).getGridParam('colModel');
	var colNameObject = {};
	for(var item of tableHeader) {
		colNameObject[item.name] = item.hidedlg||false;
	}

	var colState = []; // 只存储展示的列
	if ($checkboxs.length > 0) {
		$checkboxs.each(function() {
			// 若显示则push进colState
			$(this).prop('checked') ? colState.push($(this).attr('name')) : '';

			var colName = $(this).attr('name');
			if (!$(this).prop('checked') && !$('#' + tableId + '_' + colName).is(':hidden')) { // 若此checkbox是未选中且对应的列是显示状态，则隐藏
				if (!colNameObject[colName] || colName == "cb") {
					// 销毁冻结列
					$("#" + tableId).jqGrid('destroyFrozenColumns');
					$("#" + tableId).setGridParam().hideCol(colName);
				}
			} else if ($(this).prop('checked') && $('#' + tableId + '_' + colName).is(':hidden')) { // 若此checkbox是选中且对应的列是隐藏状态，则显示
				if (!colNameObject[colName] || colName == "cb") { // hidedlg为true的列不操作
					// 销毁冻结列
					$("#" + tableId).jqGrid('destroyFrozenColumns');
					$("#" + tableId).setGridParam().showCol(colName); 
					// 设置冻结列
	...
	// 判断是否将显示的表头存入localstorage
	var headerKey = $('#' + tableId).getGridParam('storeHeaderKey');
	if (headerKey) { // 需要存localstorage
		localStorage.setItem(headerKey, JSON.stringify(colState));
	}
}
