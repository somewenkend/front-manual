...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (seg.isStart) {
			timeText = this.getEventTimeText(event,"HH:mm",true);
			if (timeText) {
				timeHtml = '<div class="fc-time f12 pl5 pr5 pt5"><i class="fa-fw fa fa-clock-o mr5"></i>' + htmlEscape(timeText) + '</div>';
			}else{
				timeHtml = '<div class="fc-time f12 pl5 pr5 pt5"><i class="fa-fw fa fa-clock-o mr5"></i>全天日程</div>';
			}
		}
	...
	releaseScroll: function() {
		...
		if (scroll) {
			...
			if (scroll.isInitial) {
				...
				try{
					//默认不滚动，修改为获取当前选中状态的日期高度自动滚动；
					if(this.viewSpec.type == "month"){
						scroll.top=$(".fc-today").offset().top-$(calendar).offset().top-95;
						this.hardSetScroll(scroll);
					}else if(this.viewSpec.type == "listMonth" || this.viewSpec.type == "listWeek"){
						var date = new Date();
						var nowDate = $("tr[data-date='"+moment().format('YYYY-MM-DD')+"']");
						$(calendar).find("div.fc-scroller").scrollTop(nowDate.offset().top-75-$(calendar).offset().top);
					}else{
